...
def main():
    ...
    if len(argv) == 1: get_help()
    
    # Processing command line arguments: get exchange rates
    elif argv[1] == '-r':
            if len(argv) > 2: 
                for cur in argv[2:]: converter.get_xrate(cur)
            else: # Getting all avialable rates
                for cur in converter.currencies: converter.get_xrate(cur)
    
    # Processing command line arguments: currency conversion
    elif argv[1] == '-c':
        try: float(argv[2])
        except ValueError: quit('Please, enter a correct amount')
        except IndexError: get_help()
        
        if argv[3] == 'BYN': 
            try: converter.from_byn(argv[4],float(argv[2]))
            except IndexError: 
            ...
            except IndexError: converter.to_byn(argv[3], float(argv[2]))
